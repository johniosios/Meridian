...
final class HealthStore: ObservableObject {
    ...
    /// Consecutive calendar days ending today. Days whose HealthKit query fails are omitted.
    func snapshots(lastDays days: Int, now: Date = Date()) async throws -> [DatedSnapshot] {
        try await requestAuthorization()
        let cal = Calendar.current
        let today = cal.startOfDay(for: now)
        var result: [DatedSnapshot] = []
        result.reserveCapacity(days)
        for i in (0..<days).reversed() {
            guard let day = cal.date(byAdding: .day, value: -i, to: today) else { continue }
            guard let snap = try? await snapshot(for: day) else { continue }
            result.append(DatedSnapshot(date: day, snapshot: snap))
        }
        return result
    }
